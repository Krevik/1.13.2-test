...
public class BlockLayeredSand extends BaseBlock {
    ...
    public IItemProvider getItemDropped(IBlockState state, World worldIn, BlockPos pos, int fortune) {
        return Item.getItemFromBlock(KBlocks.LAYERED_SAND.getDefaultState().getBlock());
    }
    ...
    private void giveSandToNeighboursNew(IBlockState thisState, World world, BlockPos pos){
        ...
        if(layers>2){
            ...
            if(world.getBlockState(pos1).getBlock()==KBlocks.LAYERED_SAND) {
                if (getLayers(world.getBlockState(pos1))+1 < layers) {
                    world.setBlockState(pos1, KBlocks.LAYERED_SAND.getDefaultState().with(LAYERS, getLayers(world.getBlockState(pos1)) + 1), 2);
                    world.setBlockState(pos, KBlocks.LAYERED_SAND.getDefaultState().with(LAYERS, layers - 1), 2);
                    layers--;
                }
            }
            pos1 = pos.west();
    ...
    public boolean doesSideBlockRendering(IBlockState state, IWorldReader world, BlockPos pos, EnumFacing face) {
        ...
        else
        {
            ...
            return (iblockstate.getBlock() != this || iblockstate.get(LAYERS).intValue() < state.get(LAYERS).intValue()) && super.shouldSideBeRendered(state, world, pos, face);
        }
    }
