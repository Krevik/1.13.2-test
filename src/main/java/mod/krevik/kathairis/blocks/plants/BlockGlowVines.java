...
public class BlockGlowVines extends BlockKatharianPlant {
    ...
    public static final EnumProperty<EnumType> VARIANT = EnumProperty.create("variant", BlockGlowVines.EnumType.class);
        ...
        if(worldIn.getBlockState(pos).getBlock() instanceof BlockGlowVines) {
            if (worldIn.isAirBlock(pos.down())) {
                ...
                if (variant == EnumType.TOP) {
                    worldIn.setBlockState(pos.down(), KBlocks.GLOWVINES.getDefaultState().with(VARIANT, EnumType.MID));
                }
                if (variant == EnumType.MID) {
                    if (random.nextInt(5) == 0) {
                        worldIn.setBlockState(pos.down(), KBlocks.GLOWVINES.getDefaultState().with(VARIANT, EnumType.BOTTOM));
                    } else {
                        worldIn.setBlockState(pos.down(), KBlocks.GLOWVINES.getDefaultState().with(VARIANT, EnumType.MID));
                    }
    ...
    public void harvestBlock(World worldIn, EntityPlayer player, BlockPos pos, IBlockState state, @Nullable TileEntity te, ItemStack stack)
    {
        ...
        {
            spawnAsEntity(worldIn, pos, new ItemStack(KBlocks.GLOWVINES, 1));
        }
        else
        {
