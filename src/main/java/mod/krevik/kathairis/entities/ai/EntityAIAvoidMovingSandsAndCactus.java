...
public class EntityAIAvoidMovingSandsAndCactus extends EntityAIBase
{
    ...
    public boolean shouldExecute()
    {
    	...
    	for(int c=0;c<poses.size();c++) {
    		if(this.creature.world.getBlockState(poses.get(c))== KBlocks.SOFT_SAND.getDefaultState()||this.creature.world.getBlockState(poses.get(c))== KBlocks.KATHARIAN_SUCCULENT.getDefaultState()) {
    ...
    private Vec3d findPossibleShelter()
    {
        ...
        for (int i = 0; i < 3; ++i)
        {
            ...
            if (this.creature.world.getBlockState(new BlockPos(blockpos1.getX(),this.creature.world.getHeight(Heightmap.Type.MOTION_BLOCKING,blockpos1).getY()+1,blockpos1.getZ()))!=KBlocks.KATHARIAN_SUCCULENT.getDefaultState()&&this.creature.world.getBlockState(new BlockPos(blockpos1.getX(),this.creature.world.getHeight(Heightmap.Type.MOTION_BLOCKING,blockpos1).getY()-1,blockpos1.getZ()))!=KBlocks.SOFT_SAND.getDefaultState() && this.creature.getBlockPathWeight(blockpos1) < 0.0F)
            {
