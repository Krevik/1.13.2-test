...
import mod.krevik.kathairis.util.RewardHelper;
          ...
          for (int x = -radiusOfGrassPlacing / 2; x <= radiusOfGrassPlacing / 2; x++) {
              for (int z = -radiusOfGrassPlacing / 2; z <= radiusOfGrassPlacing / 2; z++) {
                  if ((x * x) + (z * z) <= (radiusOfGrassPlacing / 2 * radiusOfGrassPlacing / 2)) {
                      ...
                      world.setBlockState(tmp, KBlocks.KATHARIAN_GRASS.getDefaultState(), 2);
                      BlockPos upperPos = tmp.up();
                      if (rand.nextInt(5) == 2) {
                          world.setBlockState(upperPos, KBlocks.KATHARIAN_MINI_GRASS.getDefaultState(), 2);
                      }
