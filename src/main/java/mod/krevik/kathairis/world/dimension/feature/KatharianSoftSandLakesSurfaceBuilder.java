...
public class KatharianSoftSandLakesSurfaceBuilder implements ISurfaceBuilder<SurfaceBuilderConfig> {
            ...
            for(int k = startHeight; k >= 50; --k) {
                ...
                if (!chunkIn.getBlockState(blockpos$mutableblockpos).isAir()) {
                    if (chunkIn.getBlockState(blockpos$mutableblockpos).getBlock() != defaultFluid.getBlock()) {
                        chunkIn.setBlockState(blockpos$mutableblockpos, KBlocks.SOFT_SAND.getDefaultState(), false);
                    }
                    break;
                }
            }
