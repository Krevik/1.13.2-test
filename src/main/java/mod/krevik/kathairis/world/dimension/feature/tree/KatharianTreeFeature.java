...
public class KatharianTreeFeature extends AbstractKatharianTreeFeature {
    ...
    private static final IBlockState DEFAULT_LEAF = KBlocks.MYSTIC_LEAVES.getDefaultState();
    ...
    public KatharianTreeFeature(boolean p_i2027_1_) {
        this(p_i2027_1_, 4, DEFAULT_TRUNK, DEFAULT_LEAF, false);
    }

    public KatharianTreeFeature(boolean notify, int minTreeHeightIn, IBlockState woodMeta, IBlockState p_i46446_4_, boolean growVines) {
        super(notify);
    ...
    public boolean place(Set<BlockPos> changedBlocks, IWorld worldIn, Random rand, BlockPos position) {
        ...
        if (position.getY() >= 1 && position.getY() + i + 1 <= worldIn.getWorld().getHeight()) {
            ...
            if (!flag) {
                ...
                if (isValidGround(worldIn,position.down())&& position.getY() < worldIn.getWorld().getHeight() - i - 1) {
    ...
    public KatharianTreeFeature setSapling(net.minecraftforge.common.IPlantable sapling) {
    ...
    private boolean isValidGround(IWorld world, BlockPos pos){
        ...
        return block==KBlocks.KATHARIAN_GRASS || block==KBlocks.KATHARIAN_DIRT || block==Blocks.GRASS_BLOCK || block==Blocks.DIRT;
    }
}